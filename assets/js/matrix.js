...
const makeUniverse = size => {
  ...
  {
    for (let j=0; j < wordCloud[i].length; j++)
    {
      out.push({glyph: wordCloud[i][j], flipped: false, bright: false});
    }
  }
  return out;
};

const totalCharInWordCloud = wordCloud.join('').length;

const universe = makeUniverse(totalCharInWordCloud);
...
const setCanvasExtents = () => {
  ...
  c.textBaseline = "top";
  const charSize = c.measureText("A");
...
const rgb = "#5E81AC";
const rgbBright = "#8fbcbb";
const rgbHead = ["#88C0D0"];//, "#D0F0D0", "#80C080", "#40B040"];
const rgbTail = ["#4C566A"];//, "#003000", "#005000", "#007000"];
